      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
        ...
        <button
          onClick={loadData}
          disabled={isLoading}
          className="inline-flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-100 transition-all shadow-sm"
        >
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {corridors.length === 0 ? (
            ...
            corridors.map((c: any, idx: number) => (
              <div key={idx} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800 flex items-center justify-center shrink-0">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-xs text-slate-400 mt-1">
                      <span>Volume: <strong className="text-slate-700 dark:text-slate-300">{c.count || 1} transfers</strong></span>
                      <span>•</span>
                      <span>Avg Wait: <strong className="text-slate-700 dark:text-slate-300">{c.avgDelay || '28m'}</strong></span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-right text-[11px] text-slate-400">
                    <div>Completion: <span className="font-bold text-slate-800 dark:text-slate-200">{c.rate || '92%'}</span></div>
