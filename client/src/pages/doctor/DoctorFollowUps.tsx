...
export const DoctorFollowUps: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Overdue alert */}
      {counts.Overdue > 0 && (
        <div className="flex items-center gap-3 p-4 bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800 rounded-2xl">
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {filtered.map((fu: any) => {
            ...
            return (
              <div key={id} className={`bg-white dark:bg-slate-800 rounded-2xl border shadow-sm p-5 space-y-3 ${isOverdue ? 'border-rose-200 dark:border-rose-800' : 'border-slate-200 dark:border-slate-700'}`}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isOverdue ? 'bg-rose-50' : 'bg-blue-50 dark:bg-blue-950/30'}`}>
                      <Calendar className={`w-5 h-5 ${isOverdue ? 'text-rose-600' : 'text-blue-600'}`} />
                    </div>
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white text-sm">{fu.patientName}</p>
                      <p className="text-xs text-slate-500">{fu.reason}</p>
                      <p className="text-xs text-slate-400 mt-0.5">Due: <strong className={isOverdue ? 'text-rose-600' : 'text-slate-700 dark:text-slate-300'}>{fu.dueDate}</strong> • {fu.department}</p>
                    </div>
                  </div>
