    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={load} className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-slate-600 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 transition-all">
      ...
      {criticalCount > 0 && (
        <div className="flex items-start gap-3 p-4 bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800 rounded-2xl">
      ...
      {/* Create Form */}
      {showCreate && (
        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-teal-200 dark:border-teal-800 p-6 shadow-md">
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {orders.map((order: any) => {
            ...
            return (
              <div key={id} className={`bg-white dark:bg-slate-800 rounded-2xl border shadow-sm overflow-hidden ${order.isCritical && order.status === 'Ready' ? 'border-rose-200 dark:border-rose-800' : 'border-slate-200 dark:border-slate-700'}`}>
                <div className="px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${order.isCritical ? 'bg-rose-50' : 'bg-teal-50 dark:bg-teal-950/40'}`}>
                      {order.isCritical ? <AlertTriangle className="w-5 h-5 text-rose-600" /> : <Activity className="w-5 h-5 text-teal-600" />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-bold text-slate-900 dark:text-white text-sm">{order.testName}</p>
                ...
                {reviewing === id && (
                  <div className="px-5 pb-4 border-t border-slate-100 dark:border-slate-700 pt-3 space-y-3">
