...
export const DoctorPrescriptions: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Create Form */}
      {showCreate && (
        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-teal-200 dark:border-teal-800 p-6 shadow-md space-y-5">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input value={form.patientName} onChange={e => setForm(f => ({ ...f, patientName: e.target.value }))}
                  className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-teal-500"
            ...
            <div className="space-y-2">
              ...
              {form.items.map((item, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 items-center">
                  <div className="col-span-4"><input value={item.medicine} onChange={e => updateItem(i, 'medicine', e.target.value)} placeholder="Medicine & Strength" className="w-full px-2 py-1.5 text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg" required /></div>
                  <div className="col-span-2"><input value={item.dosage} onChange={e => updateItem(i, 'dosage', e.target.value)} placeholder="Dosage" className="w-full px-2 py-1.5 text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg" /></div>
                  <div className="col-span-3"><input value={item.frequency} onChange={e => updateItem(i, 'frequency', e.target.value)} placeholder="Frequency" className="w-full px-2 py-1.5 text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg" /></div>
                  <div className="col-span-2"><input value={item.duration} onChange={e => updateItem(i, 'duration', e.target.value)} placeholder="Duration" className="w-full px-2 py-1.5 text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg" /></div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <textarea value={form.assessment} onChange={e => setForm(f => ({ ...f, assessment: e.target.value }))} rows={2}
                  className="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-teal-500"
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {prescriptions.map((rx: any) => {
            ...
            return (
              <div key={id} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                <button
                  onClick={() => setExpanded(isOpen ? null : id)}
                  className="w-full flex items-center justify-between px-5 py-4 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors text-left"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white text-sm">{rx.patientName}</p>
                ...
                {isOpen && (
                  <div className="px-5 pb-5 border-t border-slate-100 dark:border-slate-700 space-y-4 pt-4">
                    {rx.assessment && (
                      <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl text-xs text-slate-600 dark:text-slate-300">
                        <strong className="block text-slate-800 dark:text-white mb-1">Assessment</strong>
                        {rx.assessment}
                      </div>
                    )}
                    <div className="space-y-2">
                      {items.map((item: any, i: number) => (
                        <div key={i} className="grid grid-cols-4 gap-2 p-2.5 bg-teal-50/50 dark:bg-teal-950/20 rounded-lg border border-teal-100 dark:border-teal-900/30 text-xs">
                          <span className="font-semibold text-slate-800 dark:text-white col-span-2">{item.medicine}</span>
