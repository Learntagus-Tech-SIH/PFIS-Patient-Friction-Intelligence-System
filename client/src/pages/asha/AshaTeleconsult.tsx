...
export const AshaTeleconsult: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="p-4 bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-2xl flex items-start gap-3 text-xs text-amber-900 dark:text-amber-200">
      ...
      <div className="space-y-3">
        {sessions.map((s) => (
          <div
            key={s.id}
            className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm space-y-3 hover:border-teal-300 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4"
          >
            <div className="space-y-1.5">
              <div className="flex flex-wrap items-center gap-2">
                <strong className="text-sm text-slate-900 dark:text-white">{s.patientName}</strong>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 max-w-md w-full p-6 shadow-2xl space-y-4 animate-scale-in">
            ...
            <form onSubmit={handleCreateSession} className="space-y-3 text-xs">
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Select Physician / Specialist</label>
                <select
                  value={specialist}
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Preferred Slot</label>
                <input
                  type="text"
                  value={slot}
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Chief Clinical Reason</label>
                <textarea
                  rows={2}
                  value={reason}
