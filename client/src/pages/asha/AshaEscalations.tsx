  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-300 text-xs font-bold uppercase tracking-wider mb-2">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 text-slate-600 shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
        <div className="p-4 bg-teal-50/70 dark:bg-teal-950/30 border border-teal-200 dark:border-teal-800 rounded-2xl space-y-1.5">
        ...
        <div className="p-4 bg-rose-50/70 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800 rounded-2xl space-y-1.5">
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          filtered.map((e) => (
            <div
              key={e.id}
              className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm space-y-3 hover:border-amber-300 transition-all"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex flex-wrap items-center gap-2">
                  <strong className="text-sm text-slate-900 dark:text-white">{e.patientName}</strong>
                ...
                <span className="px-3 py-1 rounded-full text-xs font-bold bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300 self-start sm:self-auto">
                  Status: {e.status}
                </span>
              </div>
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl space-y-1 text-xs">
                ...
                {e.clinicalNotes && (
                  <div className="p-2 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 rounded-lg text-emerald-800 dark:text-emerald-200 text-[11px] font-medium mt-1">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 max-w-lg w-full p-6 shadow-2xl space-y-4 animate-scale-in">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Beneficiary Name</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Escalation Category</label>
                  <select
                    value={escType}
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Specific Reason for Escalation</label>
                <textarea
                  rows={2}
                  placeholder="State the primary obstacle (e.g. 108 ambulance unable to reach due to flooded canal road)..."
                  value={reason}
                  onChange={(e) => setReason(e.target.value)}
                  className="w-full p-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl placeholder:text-slate-400"
                  required
                />
              </div>

              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Field Observations (Permitted Non-Clinical Notes)</label>
                <textarea
                  rows={2}
