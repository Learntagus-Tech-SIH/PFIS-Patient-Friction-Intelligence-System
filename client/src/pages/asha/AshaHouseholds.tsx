    ...
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-xs text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500 shadow-sm"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {isLoading ? (
          ...
          <div className="col-span-3 text-center text-slate-500 text-xs py-12 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700">
          ...
          filtered.map((h) => {
            ...
            return (
              <div
                key={h.id}
                className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm hover:border-teal-300 transition-all flex flex-col justify-between space-y-4"
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="px-2.5 py-1 rounded-lg text-xs font-bold font-mono bg-teal-50 text-teal-800 border border-teal-200 dark:bg-teal-950 dark:text-teal-200">
                      {h.householdId}
                    </span>
                    <span
                  ...
                  {/* Barriers Alert */}
                  {barriers.length > 0 && (
                    <div className="p-2.5 bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/40 rounded-xl text-[11px] text-amber-800 dark:text-amber-200 flex items-center gap-2">
                ...
                <div className="pt-3 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between text-[11px] text-slate-500">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 max-w-md w-full p-6 shadow-2xl space-y-4 animate-scale-in">
            <div className="flex items-center justify-between">
            ...
            <form onSubmit={handleCreateHousehold} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Household ID</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Family Head Name</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Total Members</label>
                  <input
                    type="number"
                    min={1}
                    value={totalMembers}
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Address / Locality</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Identified Access Barrier (Optional)</label>
                <input
                  type="text"
