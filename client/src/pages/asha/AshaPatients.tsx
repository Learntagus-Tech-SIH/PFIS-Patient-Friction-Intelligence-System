    ...
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          disabled={isLoading}
          className="px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 text-slate-700 dark:text-slate-200 text-xs font-semibold flex items-center gap-1.5 shadow-sm self-start sm:self-auto disabled:opacity-50"
        >
        ...
        <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-grow">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500"
            />
          </div>
          <button
            type="submit"
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className="p-12 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 text-center text-slate-500 text-xs">
          ...
          filtered.map((patient) => (
            <div
              key={patient.id}
              className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm hover:border-teal-300 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4"
            >
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <strong className="text-sm text-slate-900 dark:text-white">{patient.name}</strong>
                  <span className="text-xs text-slate-400">
                    ({patient.age} Yrs, {patient.gender})
                  </span>
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 flex items-center gap-1">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs text-slate-500 dark:text-slate-400">
                  <div>
                    ...
                    <strong className="text-slate-800 dark:text-slate-200">{patient.village}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[11px]">Appointment / OPD</span>
                    <strong className="text-teal-700 dark:text-teal-300">{patient.appointmentStatus}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[11px]">Follow-up Status</span>
                    <strong className="text-slate-800 dark:text-slate-200">{patient.followUpStatus}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[11px]">Identified Access Barrier</span>
                    <strong className="text-amber-700 dark:text-amber-300">{patient.accessBarrier}</strong>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-2 shrink-0 self-end md:self-center">
                <button
                  ...
                  className="px-3.5 py-2 rounded-xl text-xs font-bold border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-700 dark:text-slate-200 flex items-center gap-1.5 transition-all"
                >
      ...
      {selectedPatient && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 max-w-lg w-full p-6 shadow-2xl space-y-4 animate-scale-in">
            ...
            <form onSubmit={handleEscalateSubmit} className="space-y-4 text-xs">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl placeholder:text-slate-400"
                />
              </div>

              <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 text-[11px] text-slate-500">
                {escalationType === 'CLINICAL_CONCERN'
